...
def plot_pinn(models, metrics):
    ...
    for i in range(len(metrics)):
        u_mse = metrics[i]["u_mse"].detach().cpu().numpy()
        ax1.semilogy(iter_range, u_mse[:max_iters], label="Seed "+str(startseed))
        
        train_err = metrics[i]["train_err"].detach().cpu().numpy()
        ax2.semilogy(iter_range, train_err[:max_iters], label="Seed "+str(startseed))

        test_err = metrics[i]["test_err"].detach().cpu().numpy()
        ax3.semilogy(iter_range, test_err[:max_iters], label="Seed "+str(startseed))
        
        feas = metrics[i]["feas"].detach().cpu().numpy()
        ax4.semilogy(iter_range, feas[:max_iters], label="Seed "+str(startseed))

        mu = metrics[i]["mu"].detach().cpu().numpy()
        ax5.semilogy(iter_range, mu[:max_iters], label="Seed "+str(startseed))
    ...
    # u_mse = np.abs(metrics[i]["u_mse"].detach().cpu().numpy() - metrics[i+1]["u_mse"].detach().cpu().numpy())
    # ax1.semilogy(iter_range, u_mse[:max_iters], label="Seed "+str(startseed))
    
    # train_err = np.abs(metrics[i]["train_err"].detach().cpu().numpy() - metrics[i+1]["train_err"].detach().cpu().numpy())
    # ax2.semilogy(iter_range, train_err[:max_iters], label="Seed "+str(startseed))

    # test_err = np.abs(metrics[i]["test_err"].detach().cpu().numpy() - metrics[i+1]["test_err"].detach().cpu().numpy())
    # ax3.semilogy(iter_range, test_err[:max_iters], label="Seed "+str(startseed))
    
    # feas = np.abs(metrics[i]["feas"].detach().cpu().numpy() - metrics[i+1]["feas"].detach().cpu().numpy())
    # ax4.semilogy(iter_range, feas[:max_iters], label="Seed "+str(startseed))

    # mu = np.abs(metrics[i]["mu"].detach().cpu().numpy() - metrics[i+1]["mu"].detach().cpu().numpy())
    # ax5.semilogy(iter_range, mu[:max_iters], label="Seed "+str(startseed))
...
if __name__ == "__main__":
    ...
    plot_pinn(models, metric_dict_list)
