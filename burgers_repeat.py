...
def plot_pinn(models, metrics):
    ...
    for i in range(len(metrics)):
        ...
        ax1.semilogy(iter_range, u_mse[:max_iters], label="Seed "+str(startseed+i))
        
        train_err = metrics[i]["train_err"].detach().cpu().numpy()
        ax2.semilogy(iter_range, train_err[:max_iters], label="Seed "+str(startseed+i))

        test_err = metrics[i]["test_err"].detach().cpu().numpy()
        ax3.semilogy(iter_range, test_err[:max_iters], label="Seed "+str(startseed+i))
        
        feas = metrics[i]["feas"].detach().cpu().numpy()
        ax4.semilogy(iter_range, feas[:max_iters], label="Seed "+str(startseed+i))

        mu = metrics[i]["mu"].detach().cpu().numpy()
        ax5.semilogy(iter_range, mu[:max_iters], label="Seed "+str(startseed+i))
